...
## 1) Посилання (або flow) донату через NNS

### Висновки з дослідження
- У відкритій документації NNS немає офіційно задокументованих deep‑linkів на модальне «Send» з попередньо заповненими параметрами (адреса/сума). Джерела: офіційні гайди «NNS dapp: Authentication / Receiving ICP / Send ICP» (internetcomputer.org/docs/…)
- Реалістичні варіанти UX:
  - «Open in NNS»: лінк на `https://nns.ic0.app` (користувач логіниться через II, натискає Send, вставляє адресу — наша адреса вже у буфері обміну й у QR на сторінці донату).
...
1. Етап А (швидкий):
   - Додати кнопку «Open NNS» (відкриває `https://nns.ic0.app` у новій вкладці).
   - Автоматично копіювати `accountId` у буфер під час відкриття сторінки (з явним toast “Copied”).
   - Поруч — кнопка «Open on Dashboard» на `https://dashboard.internetcomputer.org/account/<accountId>`.
   - QR та видимий текст адреси лишаються.
2. Етап B (покращений):
...
3. Етап C (майбутнє):
   - Якщо NNS додасть deep‑link API (prefill адреси/суми), перейти на «one‑click redirect». Додати фічу‑флаг і конфіг.
